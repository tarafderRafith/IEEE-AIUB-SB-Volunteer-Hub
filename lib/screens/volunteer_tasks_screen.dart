...
if (selectedFilter == 1) {
  return tasks
      .where(
        (task) => task['status']?.toString() == 'Pending',
      )
      .toList();
}
...
if (selectedFilter == 2) {
  return tasks
      .where(
        (task) => task['status']?.toString() == 'In Process',
      )
      .toList();
}
...
return tasks
    .where(
      (task) => task['status']?.toString() == 'Done',
    )
    .toList();
...
String getAssignedByName(Map<String, dynamic> task) {
final name = task['assigned_by_name']?.toString();
...
String getAssignedByPosition(Map<String, dynamic> task) {
final position = task['assigned_by_position']?.toString();
...
String getAssignedByTeam(Map<String, dynamic> task) {
final team = task['assigned_by_team']?.toString();
                ...
                children: [
                  ...
                  Text(
                    assignedByTeam.isEmpty
                        ? assignedByPosition
                        : '$assignedByPosition • $assignedByTeam',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: const TextStyle(
                      color: Colors.white54,
